...
class Transactions extends React.Component<MyProps, IState>{
    ...
    render(){
      ...
      if(this.state.showmore){
        showmorebutton=<button className="btn btn-primary my-3 gold-btn w-100 ms-1 py-2 small-btn rounded-pill"  onClick={this.showMore}>
        More...
     </button>
      }
        ...
        {this.state.data.map((object: { blocknumber: React.Key | null | undefined; },i: string | number)=>
...
<div className="activities d-flex"  key={object.blocknumber}>
                                ...
                                {this.state.data[i].fromaddress==this.state.address?<FontAwesomeIcon icon={faArrowCircleUp} />:<FontAwesomeIcon icon={faArrowCircleDown} />}
                                </div>
                                 <div className="col-lg-9 col-7 p-3">
                                    <div className="upper-content">
                                    <b style={{color:"#A5753D"}}>{this.state.data[i].fromaddress==this.state.address?"Send":"Receive"}</b>
                                    </div>
                                    <div className="lower-content">
                                       <span> <b>{this.formatDate(this.state.data[i].time_stamp)}</b></span> - <br/>{this.state.data[i].fromaddress==this.state.address?"To":"From"} - {this.shortenAddress(this.state.data[i].toaddress)}
                                    </div>
                                </div>
